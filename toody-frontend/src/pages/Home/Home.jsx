...
const Home = ({ name }) => {
  ...
  const getTodos = async () => {
    ...
    const userToDo = await axios.get('http://localhost:3001/todos');
    setTodos(userToDo.data);
  ...
  const getNotes = async () => {
    ...
    const userJournal = await axios.get('http://localhost:3001/notes');
    setNotes(userJournal.data);
  };
